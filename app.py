...
def analyze_industry(ticker):
    ...
    try:
        ...
        【報告格式要求】
        請嚴格按照以下「LINE 手機版友善」架構輸出。
        請使用 emoji 替代傳統項目符號（如 * 或 -），確保手機閱讀體驗輕鬆、分段清晰。
        ...
        2️⃣ 關鍵題材與傳言驗證
        (請根據新聞或已知產業知識，列出 3 點市場關注的傳言或題材並驗證)
        ...
        3️⃣ 六步推論鏈 (Supply Chain Logic)
        (請建立一個邏輯嚴密的推論過程，解釋為何該公司會受惠/受害)
        ...
        4️⃣ 技術規格對決表 (Spec Match)
        (請列出該公司與競爭對手的關鍵規格對比，若無直接對手則列出前後代產品對比)
        (請勿使用 Markdown 表格，因為手機版 LINE 顯示不易，請改用以下條列格式)

        ⚔️ {stock_name} vs. [競爭對手/舊產品]
